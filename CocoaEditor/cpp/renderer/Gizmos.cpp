...
namespace Cocoa
{
	...
	void Gizmo::Render(Camera* camera)
	{
		...
		if (m_Active)
		{
			DebugDraw::AddSprite(m_TextureAssetId, m_HalfSize * 2.0f * cameraZoom, CMath::Vector2From3(m_Position), m_Tint, m_TexCoordMin, m_TexCoordMax, m_SpriteRotation);
		}
		else
		{
			DebugDraw::AddSprite(m_TextureAssetId, m_HalfSize * 2.0f * cameraZoom, CMath::Vector2From3(m_Position), { 1.0f, 1.0f, 1.0f }, m_TexCoordMin, m_TexCoordMax, m_SpriteRotation);
		}
	}
	...
	void GizmoSystem::EditorUpdate(float dt)
	{
		...
		if (!activeEntity.IsNull())
		{
			...
			for (int i = start; i < end; i++)
			{
				...
				glm::vec3 boxPos = gizmo.m_Position + CMath::Vector3From2(gizmo.m_Box2D.m_Offset) * cameraZoom;
